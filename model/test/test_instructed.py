...
def do_tts(tts_text,prompt_texts,cosyvoice,prefix):
    ...
    for i, (prompt_audio_file, prompt_text) in enumerate(zip(prompt_audios, prompt_texts)):
        ...
        with torch.no_grad():
            for j, k in enumerate(cosyvoice.inference_instruct2(tts_text,prompt_text, prompt_speech_16k, stream=False,speed=1)):
...
if __name__ == '__main__':
    ...
    do_tts('一个教授逻辑学的教授，[laughter]有三个学生，而且三个学生均非常聪明！[breath]一天教授给他们出了一个题，[breath]教授在每个人<strong>脑门上</strong>贴了一张纸条并告诉他们，每个人的纸条上都写了一个正整数，且某两个数的和等于第三个！',prompt_texts,cosyvoice,"instructed_cn")