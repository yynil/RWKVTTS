...
为了实现对生成过程的精确控制，模型词汇表中加入了一系列特殊 Token。这些 Token 在 `convert_rwkv7_to_xy.py` 脚本中被添加到分词器（Tokenizer）中，并在 `process_batch` 函数中被用来构建输入序列。

| Token 类型 | 格式 | 数量 | 用途和说明 |
|:---|:---|:---:|:---|
| **Speaker** | `[S0]` to `[S9]` | 10 | 用于指定生成语音的说话人音色。在多说话人 TTS 任务中，可以通过在 prompt 开头放置不同的 Speaker Token 来实现声音克隆。当前训练脚本默认使用 `[S0]`。 |
| **Control** | `[CTL0]` to `[CTL99]` | 100 | 作为通用的控制指令，用于切换模型模式或触发特定功能。在当前实现中，`[CTL0]` 被附加在文本 prompt 的末尾，作为一个明确的信号，标志着文本部分的结束和音频生成部分的开始。 |
| **Audio** | `(Integer ID)` | 1024 | **重要**: 音频 Token **不是** `[SPx]` 这样的具名 Token。它们是 `XY_Tokenizer` 输出的整数 ID（范围 `0-1023`）。当它们被放入混合通道（通道0）时，会被加上一个巨大的偏移量 `text_shift_size` (65536)，变为 `65536` 至 `65536+1023` 范围内的值，以和词汇表中的文本 Token 相区分。 |

### 实际应用示例

在 `process_batch` 函数中，一个原始文本 `"你好"` 会被构造成如下格式的 prompt：
...
- `[S0]`: 指示模型使用 0 号说话人的音色。
- `你好`: 原始文本内容。
- `[CTL0]`: 标志着文本结束，模型应准备开始生成与这段文本对应的音频 Token。

这种设计为未来扩展模型功能（如支持更多说话人、更多控制指令）提供了坚实的基础。
