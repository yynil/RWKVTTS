...
from model.llm.rwkv7 import RWKV_x070 as RWKV
from model.llm.rwkv7 import RWKV_x070_CMix_seq, RWKV_x070_TMix_seq, RWKV_x070_CMix_one, RWKV_x070_TMix_one, RWKV_x070_CMix_seq_batch, RWKV_x070_TMix_seq_batch
...
def forward_seq_with_embeds(model :RWKV,embeds:torch.Tensor,dtype,device,state:List[torch.Tensor] = None,return_whole_output:bool = True):
    ...
    with torch.no_grad(): 
        ...
        for i in range(model.n_layer):
            ...
            xx, v_first = RWKV_x070_TMix_seq(i, model.n_head, model.head_size, xx, state[0][i], v_first, state[1][i],
                    z[att+'x_r'], z[att+'x_w'], z[att+'x_k'], z[att+'x_v'], z[att+'x_a'], z[att+'x_g'],
                    z[att+'w0'], z[att+'w1'], z[att+'w2'], z[att+'a0'], z[att+'a1'], z[att+'a2'], z[att+'v0'], z[att+'v1'], z[att+'v2'],
                    z[att+'g1'], z[att+'g2'], z[att+'k_k'], z[att+'k_a'], z[att+'r_k'],
                    z[att+'receptance.weight'], z[att+'key.weight'], z[att+'value.weight'], z[att+'output.weight'],
...
def load_asr_models(audio_lm_path, llm_path,whisper_path,tokenizer_path,device,dtype):
    ...
    audio_llm_args = Namespace()
    audio_llm_args.MODEL_NAME = audil_lm_model_name
    audio_llm = RWKV(args=audio_llm_args)
    ...
    llm_args = Namespace()
    llm_args.MODEL_NAME = llm_model_name
    llm = RWKV(args=llm_args)
...
def inference_asr(models, audio_path, language, dtype, device, resample_count=1):
    ...
    batch_tokens = [[sample_logits(initial_logits, top_k=10, top_p=0.6, temperature=0.6)] for _ in range(resample_count)]
