...
def worker_process(process_id: int, input_queue: mp.Queue, init_done_queue: mp.Queue, output_file: str, model_dir: str):
    ...
    try:
        ...
        # 初始化tokenizer
        audio_tokenizer = load_higgs_audio_tokenizer(model_dir,gpu_id)
        # 发送初始化完成信号
...
def main():
    ...
    parser.add_argument('--model_dir', type=str, default='/home/yueyulin/models/higgs-audio-v2-tokenizer/')
    ...
    try:
        ...
        for i, item in enumerate(tqdm(dataset, desc=f"Processing Tars from {dir_name}")):
            ...
            audio_field_names = ['wav','mp3','flac']
            audio_key = None
            for audio_field_name in audio_field_names:
                if audio_field_name in item:
                    audio_key = audio_field_name
                    break
            if audio_key is None:
                print(f"No audio field found in {item}")
                continue
            audio_buffer = io.BytesIO(item[audio_key])
            wav, sample_rate2 = librosa.load(audio_buffer,mono=True,sr=None)
            
            data = (json_data, wav, sample_rate2)
