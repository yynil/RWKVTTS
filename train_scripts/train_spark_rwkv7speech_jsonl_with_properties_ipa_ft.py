...
from utils.phonem_utils import ramdomly_mark_phonem
...
def main():
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            ...
            # 增强样本（仅替换 text 字段）
            text = sample['text']
            # 简单按文本内容推断整体语言：含中文优先判为中文
            lang = 'zh' if re.search(r'[\u4e00-\u9fff]', text) else 'en'
            marked_text = ramdomly_mark_phonem(text, lang=lang, min_mark=1, max_mark=3)
            aug_sample = dict(sample)
            aug_sample['text'] = marked_text
            augmented.append(aug_sample)

        keys = augmented[0].keys()
        return {key: [d.get(key) for d in augmented] for key in keys}
