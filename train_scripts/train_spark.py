...
def collate_fn_audio(batch, tokenizer, max_length, pad_to_max_length, semantic_start_id, audio_tokenizer):
    """处理音频数据的 collate 函数"""
    texts = []
    global_tokens_list = []
    ...
    for item in batch:
        # 处理音频数据
        audio_data = np.array(item['mp3']['array'], dtype=np.float32)
        sampling_rate = item['mp3']['sampling_rate']
        
        # 重采样如果需要
        if sampling_rate != audio_tokenizer.config['sample_rate']:
            from librosa import resample
            audio_data = resample(audio_data, orig_sr=sampling_rate, target_sr=audio_tokenizer.config['sample_rate'])
    ...
    # 找到 semantic_start 的位置
    semantic_start_positions = []
    for i, input_ids in enumerate(encodings['input_ids']):
        semantic_start_pos = (input_ids == semantic_start_id).nonzero(as_tuple=True)[0]
        if len(semantic_start_pos) > 0:
            semantic_start_pos = semantic_start_pos[0]
            semantic_start_positions.append(semantic_start_pos)
            
            # 将 semantic_start 之后的位置替换为 semantic_tokens
            semantic_tokens_tensor = torch.tensor(semantic_tokens_list[i], dtype=torch.long)
            labels[i, semantic_start_pos+1:semantic_start_pos+1+len(semantic_tokens_tensor)] = semantic_tokens_tensor
            
            # 将剩余位置设为 -100
            labels[i, semantic_start_pos+1+len(semantic_tokens_tensor):] = -100
...
def main():
    ...
    if args.webdataset_dir is not None:
        if args.audio_model_dir is None:
            raise ValueError("audio_model_dir must be set when webdataset_dir is provided")
        if is_main_process:
            logger.info(f"Initializing audio tokenizer from {args.audio_model_dir}")
        audio_tokenizer = BiCodecTokenizer(args.audio_model_dir, device=device)
        audio_tokenizer.eval()  # 设置为评估模式
    ...
    if args.webdataset_dir is not None:
        ...
        import glob
        all_tars = glob.glob(os.path.join(args.webdataset_dir, '*.tar'))
        all_tars.sort()
        ...
        for tar_file in all_tars:
            try:
                # 使用正则表达式匹配文件名末尾的6位数字
                import re
                match = re.search(r'(\d{6})\.tar$', tar_file)
                if match:
                    index = int(match.group(1))
                    if args.from_index <= index < args.to_index:
                        filtered_tars.append(tar_file)
                else:
                    logger.warning(f"Could not find 6-digit index in filename: {tar_file}")
            except (ValueError, IndexError) as e:
                logger.warning(f"Error processing filename {tar_file}: {str(e)}")
                continue
        
        if not filtered_tars:
            raise ValueError(f"No tar files found in index range [{args.from_index}, {args.to_index})")
        
        logger.info(f"Found {len(filtered_tars)} tar files in index range [{args.from_index}, {args.to_index})")
